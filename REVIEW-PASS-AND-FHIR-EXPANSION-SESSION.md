...
- **Session 2 — Close the three Part 0 gap items.** Scope: `form_encounter` DB test, `lists` DB test, Appointment update/cancel/recurring (Part 0 below).
  **Status: not started.** Start here: read Part 0, then `CLAUDE.md`'s DB/API test conventions, then follow the suggested pattern reuse noted in Part 0 for each of the 3 items. Optional, not required: Session 1 left one open architectural question (the `AuditLogDbTests` race described in Part A5) that isn't part of this session's assigned scope — pick it up only if there's spare time, otherwise leave it for whoever does Session 4.
- **Session 3 — FHIR expansion (Part B).** Scope: Encounter, AllergyIntolerance, Condition, MedicationRequest, Observation search+read, in the order Part B recommends.
  **Status: not started.** Depends on Session 1's result (done, confirmed safe) — no further fresh-container verification needed before starting this one.
...
## 0. Two gaps from the original plan, owned up front
...
- **`form_encounter` DB test** (every encounter row has a valid `pid` and `provider_id`) — still `[ ]` in `TEST-PLAN.md`. This was Track B item 1, explicitly called out as having no dependency on the API work, and it just never got picked up.
- **`lists` DB test** (allergy/immunization code lists match UI dropdowns) — still `[ ]`. Track B item 6, lowest priority in that group, which is presumably why it fell off, but it's still open.
- **Appointment: update/reschedule, cancel, recurring series** — still `[ ]` in `TEST-PLAN.md`'s Layer 1 REST section. This one's on me specifically: it was in `TEST-PLAN.md` before the buildout started and I never sequenced it into the 11-item list in `CSHARP-BUILDOUT-SESSION.md` at all.

Recommend folding all three into whichever of Part A / Part B below runs first, since they're small and independent of everything else in this file.
