...
**Why:** Session 1b's investigation confirmed something genuinely interesting: the Calendar click's `menuActionClick` handler *partially* works even when stuck — the iframe's `src` does reload with real content (a real network request fires, confirmed in Session 1's original probe) — but the "make this tab's `div.frameDisplay` visible" part of the same handler never completes. That's the signature of a Knockout computed/subscription silently failing partway through an update (Knockout swallows exceptions thrown inside dependency chains without necessarily surfacing them anywhere obvious), not a click that's blocked or ignored outright. This specific check — watching for a thrown-but-swallowed JS exception during the exact failing click — was named in Session 1b's original plan but never actually executed; every probe run so far filtered console output down to specific expected messages rather than capturing all errors and page-level exceptions.

**Do:**
1. Reproduce the full failing flow one more time, but this time attach `page.on('pageerror', ...)` and an unfiltered `page.on('console', msg => ...)` (capturing `msg.type() === 'error'` specifically) *before* the credentials-modal step, and watch for anything thrown at the moment of the Calendar click specifically (correlate by timestamp).
2. If an exception surfaces, trace it back to source (this project's established discipline — read the actual JS, don't guess) and determine whether it's fixable from the test side (e.g., some state the test could reset before clicking) or is a genuine, unfixable-from-outside OpenEMR bug worth a `FINDINGS.md` entry.
3. If nothing surfaces even with full unfiltered error capture, that's a real, meaningful negative result too — it would mean the failure is happening even more silently than a swallowed exception (e.g., a Knockout subscription that was never wired up correctly after the modal's own DOM manipulation ran, not an exception at all), and at that point this is probably at the limit of what black-box investigation can determine without OpenEMR's own source-level debugger access. Say so plainly rather than continuing to guess at further theories.

**Definition of done:** Either a concrete, source-confirmed root cause (and a real fix, verified via repeated trials) or an honest, final "investigated to the reasonable limit of black-box testing, here's everything ruled out" write-up. Given how much this specific bug has already been narrowed down across three sessions (1, 1b, 1c), if this comes up empty too, that's a reasonable point to stop returning to it rather than continuing indefinitely — note that explicitly if it happens.
