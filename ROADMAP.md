...
`CONTEXT.md`'s Purpose & Vision item 3 scoped this phase to cover race conditions, partial failures, *and* non-concurrency timing-dependent bugs. All three sub-areas now have at least one confirmed result (race conditions/concurrency: `FINDINGS.md` #18-22, #24, #27; partial failures: #25; timing-dependent bugs: #26) — this group's sessions below are either narrower follow-ups on an already-covered sub-area (2b, 4b) or were the original session that first covered one (3, 4, 5, 6), not a signal that the sub-area itself is still untouched.

### Session 3: Facility concurrent-create race test
**Status:** Done (confirmed defect, matches Practitioner's unguarded shape — see outcome below)

**Outcome (2026-08-04):** Read `FacilityService::insert()`/`FacilityValidator` and `SHOW CREATE TABLE facility` before writing any test, per this project's "confirm, don't guess" standard. `facility.id` turned out to be a genuine `AUTO_INCREMENT` column (not an application-computed value like `patient_data.pid`), and the only other constraint is a `UNIQUE KEY` on `uuid` (always distinct per insert). No unique index on `name`/`facility_npi`/anything else, and `FacilityValidator`'s insert context only requires `name` and `facility_npi` to be present and well-formed — never checked against existing rows. This is the fully-unguarded shape, not the guarded-but-mishandled one: added `Concurrent_Facility_Creates_With_Identical_Name_And_Npi_All_Succeed_With_No_Uniqueness_Guard` to `ConcurrencyApiTests.cs` (5 concurrent creates with identical name/NPI), confirmed all 5 succeed with distinct ids/uuids, passed first try, full suite re-run clean twice. Written up as `FINDINGS.md` #24, following the #21 (Practitioner) format exactly. `TEST-PLAN.md`'s Layer 4 section updated; defect counts in `README.md`/`ROADMAP.md` bumped from 22 to 24.
...
### Session 4b: Root-cause the document-upload concurrent-load 404s that Session 4 first wrote off as a cold-start anomaly
**Status:** Done (root cause found and confirmed — see outcome below)

**Outcome (2026-08-05):** Read `DocumentService::insertAtPath()` and, past it, `Document::createDocument()` (`library/classes/Document.class.php`) — the actual failing branch was never one of the three "early guard clause" messages the previous session grepped the Apache log for; it's a fourth, later message (`"There was an error in attempt to upload a patient document"`, from `insertAtPath()`'s `if (!empty($ret))` check on `createDocument()`'s own return value) that was never checked. `createDocument()` lazily creates each patient's per-patient document storage directory on their first-ever upload via a plain, unguarded `if (!file_exists($filepath)) { mkdir($filepath, 0700, true); }` — a textbook TOCTOU race with no lock. Grepping the log for the correct fourth message found exact matches at both real incidents' timestamps, with hit counts matching each incident's failure count exactly (6 hits at the first incident, 8 at the second). Confirmed live in two ways: raising the existing test's concurrency well past 10 (a throwaway 40-concurrent-upload probe against one new patient, run and then deleted) still never reproduced it even once over real HTTP, showing Apache/PHP-FPM's own per-request dispatch overhead keeps genuinely isolated bursts from landing in the same microsecond window; then, bypassing HTTP entirely, several real PHP processes synchronized on a shared release-barrier file (so their `file_exists()` calls land within microseconds of each other) reproduced the exact "one `mkdir()` wins, the rest get PHP's own File-exists failure" race every time against a freshly-removed directory — a one-off diagnostic technique, not committed to the suite, since it steps outside this project's black-box-HTTP architecture. Written up as `FINDINGS.md` #27; `CONTEXT.md`'s decision-log entry and `TEST-PLAN.md`'s Layer 4 line both updated from "not yet root-caused" to the confirmed mechanism; defect counts in `README.md` bumped 26→27 (9→10 "additional" grey-area defects). No fix applied to OpenEMR itself, per this project's standing scope.

**Why:** Session 4's own full-suite re-runs were clean, but the *first* full-suite run right after a `docker compose` restart wasn't — 6 of 10 concurrent uploads returned `404`, written off at the time as PHP-FPM/MariaDB still warming up. It then happened again, independently, hours into an already-long-running, fully warm container (8 of 10 failed that time), which rules the cold-start theory out. This is a real, recurring, load-dependent failure mode, not a one-off — see `TEST-PLAN.md`'s updated entry for the full trail.

**Do:**
1. Both observed failures share an identical shape: some concurrent uploads return `404` with a literal `null` body (`RestControllerHelper::responseHandler()`'s generic "falsy service result" branch), meaning `DocumentService::insertAtPath()` returned `false`. Directly querying `documents`/`categories_to_documents` for the second failure's patient confirmed the failing requests never created a `documents` row at all (not even an orphaned file) — ruling out the DB-insert/category-link stage and pointing at one of `insertAtPath()`'s three early guard clauses: `secure_upload`/`isWhiteFile()`, `isValidPath()`, or `empty($file)` after `file_get_contents()` on the request's temp upload file.
2. All three of those branches call `error_log()` with a distinctive message, and none of those messages showed up in the Apache error log for either failure — including after catching a real log-rotation gotcha (`error.log` vs. `error.log.1`; make sure to check both/whichever actually covers the failure's timestamp window before concluding the log has nothing). Either the correlation missed the right window, or the falsy return isn't actually coming from one of those three documented branches at all — worth adding a temporary diagnostic (e.g., a one-off `error_log()` at each guard clause, or capturing PHP's own `microtime()` alongside the C# test's request timestamps) to nail down exactly which branch fires, or whether it's something upstream in `DocumentRestController::postWithPath()`/`_rest_routes.inc.php`'s multipart-file handling instead.
3. Since this only reproduces under genuine full-suite concurrent load (never in isolation, confirmed via repeated isolated runs both times), reproduction requires running the full suite, not just this test's own class — budget for that.

**Definition of done:** Either a real, source-confirmed root cause (write it up as a new `FINDINGS.md` entry, following the existing format) or, if it turns out to be a genuine environmental/infrastructure limitation rather than an application defect (e.g., a PHP-FPM worker-count ceiling under this specific docker-compose config), an honest note saying so in `CONTEXT.md`'s Known Constraints and in `TEST-PLAN.md`'s entry — but not another "probably a one-off" guess without stronger evidence than Session 4b started with.

---

### Session 5: Partial-failure scenario testing (new sub-phase)
**Status:** Done (confirmed defect — see outcome below)

**Outcome (2026-08-05):** Chose `merge_patients.php` per this session's own top-ranked candidate, but had to find a real, non-concurrency failure trigger rather than reuse `FINDINGS.md` #22's race — reading the loop directly (rather than assuming from #22's summary) surfaced that it doesn't touch a fixed table list at all, it runs `SHOW TABLES` and blindly `UPDATE`s any table with a `pid`/`patient_id` column. Reading `sqlStatement()`/`HelpfulDie()` confirmed any single failing statement anywhere in that loop calls PHP's bare `exit` — no catch, no rollback (there's nothing to roll back anyway, per #22's already-known missing transaction). Queried `information_schema.KEY_COLUMN_USAGE` for a table with both a `pid` column and a real unique constraint to engineer a concrete, plausible collision rather than guess at one; `patient_access_onsite`'s single-column `UNIQUE KEY (pid)` was the cleanest match — one portal-access row per patient is an entirely ordinary real-world state, not a contrived fixture. Seeded that collision plus a `lists` row (sorts alphabetically before, in `SHOW TABLES` order) and a `pnotes` row (sorts after) to prove the exact migrated/not-migrated boundary. New spec `ui/tests/admin-duplicate-merge-partial-failure.spec.ts`, reusing `admin-duplicate-merge-race.spec.ts`'s established CSRF-scrape-then-POST pattern — passed cleanly on both browsers in isolation (repeated runs); when run alongside its sibling spec files it occasionally hits the same pre-existing `confirmDuplicateCheck()` popup timing flakiness already documented elsewhere in this project (not a new flaky spot). Written up as `FINDINGS.md` #25 — a materially worse outcome than #22's race, since it's reachable by one ordinary request with no timing required, and leaves the source patient fully existing (so nothing flags to an operator that anything went wrong) while part of its clinical data has already moved to the target. `CONTEXT.md`'s Decision Log and Purpose & Vision line updated; `TEST-PLAN.md`'s Layer 4 section (retitled to include "partial failures") updated; defect count bumped 24→25 in `README.md`/`ROADMAP.md`.
...
**Outcome (2026-08-05):** Picked candidate 1 (appointment date/time boundary) first, since it was the only one of the four directly testable at the API layer with a single request and no timezone/DST dependency. Reading `AppointmentService::insert()` found the bug on the first pass: it correctly computes the appointment's real end `DateTime` including a day rollover when `pc_duration` pushes past midnight, but then stores only `->format('H:i:s')` into `pc_endTime` — a plain `TIME` column with no date component — discarding the rollover; `pc_eventDate` is never adjusted either. Confirmed live before writing any assertion: a 23:00 start with a 2-hour (`7200`s) duration produced a real row with `pc_eventDate` unchanged, `pc_startTime=23:00:00`, `pc_endTime=01:00:00` — an end time earlier than its own start time — verified via both the REST response and a direct query against `openemr_postcalendar_events`. Written up as `FINDINGS.md` #26 and encoded as `Appointment_Duration_Crossing_Midnight_Stores_A_Time_Only_EndTime_With_No_Date_Rollover` in a new `tests/OpenEmr.Api.Tests/GreyArea/TimingBoundaryApiTests.cs`. Stopped at one confirmed finding per this session's own "pick one or two" scoping — candidates 2-4 (OAuth clock-skew, audit-log timestamp ordering, recurring-series boundary) remain untried. `CONTEXT.md`'s Purpose & Vision and Decision Log updated (all three grey-area sub-areas now have at least one confirmed result); `TEST-PLAN.md`'s Layer 4 section updated; defect count bumped 25→26. **A full-suite run during this same session also re-triggered `ConcurrencyApiTests.cs`'s document-upload `404`s from Session 4/4b — see Session 4b, opened this session to track that properly instead of leaving the previous "one-off anomaly" conclusion uncorrected.**
...
- **Fixing the 27 `FINDINGS.md` defects themselves** — deliberately out of scope per this project's own framing (`CONTEXT.md`: "OpenEMR is the system under test, not this repo's own code"). This project's job is to find and document real defects, not patch a third-party production system's source. Don't spend a session "fixing" `MessageService::update()`'s missing `pid` filter, for example — that's not what this repo is for.
