...
Layer 3 (UI) is partial. `auth.spec.ts`, `patient-registration.spec.ts`, `billing-payment.spec.ts`, and now `patient-scheduling.spec.ts` (6 tests: the original 3 plus reschedule, weekly recurring series, and provider-filter fallback behavior) are reliably green, modulo the pre-existing patient-registration duplicate-check dialog flakiness under parallel load (retries absorb it). Several UI areas in `TEST-PLAN.md` Layer 3 remain unbuilt: clinical encounter (SOAP note, sign/lock), billing claim generation, RBAC in the UI, patient portal, and the admin duplicate-merge tooling. The four "outside the three core layers" items (accessibility, load/perf, contract/schema validation, security/negative) and test-data-lifecycle automation are also still open.
...
1. Continue `TEST-PLAN.md`'s remaining open items: clinical encounter (SOAP note, sign/lock), billing claim generation, RBAC in the UI, patient portal, admin duplicate-merge tooling (`manage_dup_patients.php`/`merge_patients.php` — no exploration done yet). FHIR `$everything`/Bundle transaction POST and UI reschedule/recurring/provider-filtering are now done — see `CONTEXT.md` for what's confirmed supported vs. genuinely absent in this OpenEMR version (drag-and-drop reschedule doesn't exist; true multi-provider filtering isn't testable without first building a second provider account, deliberately not done this session).
