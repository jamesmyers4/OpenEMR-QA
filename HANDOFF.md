...
4. The "grey area" reliability-testing phase described in `CONTEXT.md` (race conditions, partial failures, timing-dependent bugs) has now started — see the 2026-07-27 follow-up above and `TEST-PLAN.md`'s new Layer 4 section for the first three confirmed races (`FINDINGS.md` #18/#19). This was deliberately scoped narrow rather than exhaustive; candidates not yet attempted include races on other resources with the same missing-locking shape (Patient Insurance's destructive `PUT`, Practitioner/Facility creates), partial-failure scenarios (a request that fails partway through a multi-table write), and timing-dependent bugs beyond pure concurrency. `ConcurrencyApiTests.cs`/`ConcurrencyDbTests.cs`'s `Task.WhenAll` pattern (and the DB test's deterministic forced-interleaving technique, which sidesteps relying on `Task.WhenAll` timing luck) are the templates to extend.
