...
1. Continue `TEST-PLAN.md`'s remaining open items: UI reschedule/drag-drop/recurring/provider-availability, clinical encounter, billing claim generation, RBAC in the UI, patient portal, admin duplicate-merge tooling (`manage_dup_patients.php`/`merge_patients.php` — no exploration done yet). FHIR `$everything` and Bundle transaction POST are now done — both confirmed unsupported by this OpenEMR version, see `CONTEXT.md`.
