...
## 26. An appointment whose duration pushes past midnight is stored with an end time earlier than its own start time (COMPLETED)

**Severity:** Medium
**Status:** Open
**Component:** `AppointmentService::insert()` (`src/Services/AppointmentService.php`), backed by `openemr_postcalendar_events.pc_endTime` (a plain `TIME` column)

**Summary:** `AppointmentService::insert()` correctly computes the appointment's real end date and time as a full `DateTime` — adding `pc_duration` seconds via `DateInterval` genuinely rolls over into the next calendar day when the duration pushes past midnight, exactly as `DateTime::add()` should. But the code then stores only `$endTime->format('H:i:s')` into `pc_endTime`, which is itself a bare `TIME` column with no date component at all — discarding the date rollover completely. `pc_eventDate` is copied verbatim from the request and is never adjusted either. The result: a real, plausible booking (an overnight visit, an on-call shift, any appointment starting late enough that its duration crosses midnight) is stored with an end time that is literally *earlier* than its own start time on the same nominal date, with nothing anywhere in the schema recording that the appointment actually spans two calendar days.

**Repro:** `POST /api/patient/{pid}/appointment` with `pc_startTime = "23:00"`, `pc_duration = "7200"` (2 hours), `pc_eventDate` any future date `D`. The create succeeds normally (`200`). `GET /api/appointment/{eid}` afterward shows `pc_eventDate = D`, `pc_startTime = 23:00:00`, `pc_endTime = 01:00:00` — confirmed directly against the live `openemr_postcalendar_events` row as well, matching the API response exactly.

**Root cause:** Confirmed by reading `AppointmentService::insert()` directly: `$endTime = (new \DateTime())->setTimestamp($startUnixTime)->add($endTimeInterval);` correctly produces a `DateTime` on the *next* calendar day when duration crosses midnight, but the SQL bind for `pc_endTime` is `$endTime->format('H:i:s')` — a plain time string with the date component thrown away — and `pc_eventDate` is bound straight from `$data["pc_eventDate"]`, the caller's original date, untouched. `pc_endTime`'s own column type (`TIME`) means there is no way to store the correct value even if the code wanted to — the schema itself has no field capable of recording "ends the following day."

**Impact:** Any downstream logic that compares `pc_startTime` and `pc_endTime` naively (duration/overlap calculations, calendar rendering, double-booking checks) will see an end time before the start time for any appointment that legitimately crosses midnight, rather than a duration that happens to be long. This is a real scheduling-data-integrity gap for a healthcare-adjacent domain where overnight/on-call appointments are a normal case, not an edge case invented for testing purposes.

**Automated coverage:** `Appointment_Duration_Crossing_Midnight_Stores_A_Time_Only_EndTime_With_No_Date_Rollover` (`tests/OpenEmr.Api.Tests/GreyArea/TimingBoundaryApiTests.cs`) creates a 23:00-start, 2-hour appointment and asserts `pc_eventDate` is unchanged and `pc_endTime` parses to a time strictly before `pc_startTime` — the concrete, checkable signature of this defect.

---

## 27. Concurrent uploads of a patient's first-ever document race on an unguarded directory-creation check and silently lose the upload with a bare `404` (COMPLETED)

**Severity:** Medium
**Status:** Open
**Component:** `Document::createDocument()` (`library/classes/Document.class.php`), reached via `DocumentService::insertAtPath()` (`src/Services/DocumentService.php`)

**Summary:** Every patient's uploaded documents are stored under a single per-patient directory (`{repository}/{pid}/`), created lazily on that patient's first-ever upload via a plain `if (!file_exists($filepath)) { mkdir($filepath, 0700, true); }` — a classic check-then-act race with no lock around it. When two or more uploads for a patient with no prior documents race each other, every one of them can pass the `file_exists()` check before any of them finishes `mkdir()`; PHP's `mkdir()` returns `false` (with an `E_WARNING`, "File exists") for every caller except whichever one's syscall physically wins, even with the recursive flag set, because the deepest directory component already exists by the time the losing calls run. `createDocument()` treats that `mkdir()` failure exactly like any other storage fault — it returns the string `'Unable to create patient document subdirectory'` instead of writing anything — and `insertAtPath()`'s `if (!empty($ret))` branch turns that into a logged error and a bare `false`, which `RestControllerHelper::responseHandler()` renders as `404` with a `null` body: no file written, no `documents` row created, and nothing in the response distinguishing this from any other generic failure.

**Repro:** This is the same underlying defect behind two previously-unexplained live incidents already noted in `TEST-PLAN.md`/`CONTEXT.md`: a first full-suite run right after a fresh `docker compose` restart where 6 of 10 concurrent uploads to one brand-new patient's document endpoint returned `404`/`null`, and a second, independent occurrence hours into an already-warm container where 8 of 10 failed the same way. Both had been provisionally written off as environment warm-up noise. Grepping the container's Apache error log for this defect's own distinctive message (`"There was an error in attempt to upload a patient document"` — a message downstream of `insertAtPath()`'s three already-checked early guard clauses, not one of them) finds an exact match for both incidents: 6 hits clustered at `2026-08-04 22:56:00.550` (matching the first incident's 6 failures) and 8 hits clustered at `2026-08-05 13:23:19.079` spanning under 150 microseconds end to end (matching the second incident's 8 failures) — direct log-level confirmation this is the real mechanism behind both, not a coincidence. The exact race was then reproduced directly and deterministically outside of HTTP entirely: several genuinely concurrent PHP processes (synchronized on a shared release-barrier file so their `file_exists()` calls land within microseconds of each other, not dispatched via HTTP/Apache where request/connection overhead reliably staggers them enough to avoid the race) all targeting one freshly-`rm -rf`'d directory — repeatedly, only one process's `mkdir()` ever returns `true`; every other one that also observed `file_exists() === false` gets `mkdir_result=false` plus the exact `PHP Warning: mkdir(): File exists` PHP itself emits.

**Root cause:** `Document::createDocument()`'s directory-creation block (`library/classes/Document.class.php`) does an unguarded `file_exists()` check followed by a separate `mkdir($filepath, 0700, true)` call, with no locking, advisory or otherwise, between the two — a textbook TOCTOU (time-of-check-to-time-of-use) race. `insertAtPath()` only guards against three earlier, well-behaved failure modes (unsafe file type, invalid category path, empty upload) before calling `createDocument()`; it has no special handling for `createDocument()`'s own directory-creation failure, so a `mkdir()` race collapses into the same generic `error_log()` + `return false` path as any other storage error. Confirmed live to require genuine OS-level concurrency to trigger — sending the same burst of concurrent requests over real HTTP, even at higher concurrency (40 simultaneous uploads in a single isolated test run) than the original 10-upload test uses, never reproduced it even once, since Apache/PHP-FPM's own per-request dispatch overhead is apparently enough to keep `file_exists()` calls from truly landing in the same microsecond window under that path — matching why this was never caught by the existing test running in isolation, only under genuine full-suite concurrent load contending for the same container's limited worker pool.

**Impact:** A real, silent data-loss mode for any workflow that uploads a patient's first document under real concurrent load — a batch import, multiple portal uploads landing close together, or simply enough unrelated traffic hitting the same OpenEMR instance at once. Unlike several other findings in this list, the caller here does get an honest failure signal (`404`, not a false `200`), but the response gives no indication of why, and nothing about the request itself was actually invalid — a retry with the exact same payload would very likely succeed, since the directory will exist by then. Any table/document ever added for a patient's very first upload is a fresh trigger for this, not a one-time occurrence per patient.

**Automated coverage:** `Concurrent_Document_Uploads_To_Same_Existing_Category_All_Succeed_And_Stay_Independently_Listable` (`ConcurrencyApiTests.cs`) already exercises the exact vulnerable code path (a brand-new patient's very first upload, uploaded to concurrently) and is what caught both real incidents underlying this finding — but true reproduction needs genuine OS-level scheduling contention, not just a large number of concurrent HTTP requests from one isolated test process (confirmed above), so this test will continue to pass most of the time in isolation and only intermittently catch the real race under genuine full-suite/production-like load, the same load-dependent shape already documented for this test in `TEST-PLAN.md`. No committed test drives the direct-PHP barrier-synchronized reproduction described above, since it steps outside this project's black-box-HTTP testing architecture (`CONTEXT.md`'s Purpose & Vision: OpenEMR is tested as a black box from the outside) — it was a one-off diagnostic technique to confirm the mechanism, not permanent test infrastructure.

---

## 28. Any Observation FHIR search crashes with a `500` as soon as a single Vitals record exists anywhere in the database — not just for that patient (COMPLETED)

**Severity:** Critical
**Status:** Open
**Component:** `FhirObservationVitalsService::parseVitalsIntoObservationRecords()` (`src/Services/FHIR/Observation/FhirObservationVitalsService.php`), `UuidRegistry::populateAllMissingUuids()`/`UuidMapping::createAllMissingResourceUuids()` (`src/Common/Uuid/UuidRegistry.php`, `src/Common/Uuid/UuidMapping.php`)

**Summary:** Each individual vital sign (blood pressure, weight, pulse, etc.) is exposed as its own separate FHIR Observation, sharing one `form_vitals` row but needing its own distinct uuid — resolved via a `uuid_mapping` table lookup keyed by the vitals row's own uuid. Nothing in this OpenEMR version ever populates that lookup table for a `form_vitals` row, through any code path the application actually executes — including the real, normal, validated way a vitals form gets saved (`VitalsService::saveVitalsArray()`, reached by both the classic UI form and the REST `POST /api/patient/{pid}/encounter/{eid}/vital` endpoint). The one function that *would* backfill `uuid_mapping` (`UuidRegistry::populateAllMissingUuids()` → `UuidMapping::createAllMissingResourceUuids()`) is never called anywhere else in the codebase — confirmed via a full-codebase grep that returns zero results outside the two files that define it. The result: `FhirObservationVitalsService::parseVitalsIntoObservationRecords()` looks up that always-empty mapping and, for every individual vital-sign code (unlike the one "vitals panel" code just above it in the same method, which is correctly guarded with `!empty(...)`), passes the missing lookup result straight into `UuidRegistry::uuidToString()` with no `isset`/`empty` check at all — an uncaught `TypeError` (`Ramsey\Uuid\Uuid::fromBytes(): Argument #1 ($bytes) must be of type string, null given`) that PHP has no handler for, producing a bare `500` with an empty body. Since the crash happens while resolving `uuid_mapping` for *whichever* vitals record happens to match the query — not specifically the one the caller asked about — a single vitals record for any patient in the whole system permanently breaks Observation search for every patient, not just its own.

**Repro:** Create a patient and encounter, then a `form_vitals` row (with realistic, non-default field values and a real encounter — not the `encounter = 0` used in the initial, inconclusive attempt at this) plus its linking `forms` row (mirroring exactly what `addForm()` itself would insert). `GET /fhir/Observation?patient={puuid}&category=vital-signs` returns a bare `500` with an empty body. Critically, so does a completely unfiltered `GET /fhir/Observation` and a category-only `GET /fhir/Observation?category=vital-signs` with no patient filter at all — confirmed live: as long as any vitals record exists anywhere in the database, every Observation search crashes, including ones for entirely unrelated patients with no vitals data of their own. The Apache error log confirms the exact mechanism with a full stack trace: `OpenEMR.ERROR: FhirVitalsService->parseVitalsIntoObservationRecords() Cannot return vitals panel as mapping uuid is missing for code 85353-1` (the one guarded branch, correctly logging instead of crashing), immediately followed by `PHP Warning: Undefined array key "85353-1"` at `FhirObservationVitalsService.php:373`, then `PHP Fatal error: Uncaught TypeError: Ramsey\Uuid\Uuid::fromBytes(): ... null given ... in UuidRegistry.php:281`, with a stack trace running `UuidRegistry::uuidToString()` → `FhirObservationVitalsService::parseVitalsIntoObservationRecords()` (line 373) → `parseVitalsIntoObservationRecords()`'s caller → `FhirServiceBase::getAll()` → `FhirObservationService::getAll()` → `FhirObservationRestController::getAll()` → the registered route → `dispatch.php`.

**Root cause:** Two compounding gaps. First, `UuidMapping::createAllMissingResourceUuids()` — the only code in this OpenEMR version that would ever populate a `uuid_mapping` row for a `form_vitals` record — is dead code from the application's own perspective: nothing calls its only caller, `UuidRegistry::populateAllMissingUuids()`, anywhere else in the codebase (confirmed via `grep -rn` across the full application source, zero matches outside the two defining files), so it can only ever run if invoked manually and out-of-band, not as part of any real save or request flow. Second, `FhirObservationVitalsService::parseVitalsIntoObservationRecords()`'s per-individual-vital-sign loop (as opposed to the "vitals panel" special case immediately above it in the same method, which correctly checks `!empty($uuidMappings[self::VITALS_PANEL_LOINC_CODE])` before using the value) has no equivalent guard around `UuidRegistry::uuidToString($uuidMappings[$code])` — an unconditional array access into a lookup that, in practice, is always empty for every vitals record this application ever creates.

**Impact:** This is not a narrow edge case reachable only through a contrived raw-SQL fixture — the exact same crash was independently confirmed to be reachable through this API's own real, validated, application-mediated vitals-creation path (`EncounterService::insertVital()` → `VitalsService::save()` → `saveVitalsArray()` → `addForm()`), which never populates `uuid_mapping` either. In a real deployment, the very first vitals ever recorded for any patient — a completely ordinary clinical workflow event — permanently breaks the entire Observation FHIR resource for every patient, not a `404`/soft-failure but a hard `500` with no diagnostic information in the response body, for every caller of this resource from that point forward. Given `vital-signs` is one of the most commonly integrated FHIR categories for any downstream clinical-data consumer, this effectively takes the whole Observation resource offline the first time OpenEMR is used the way it's designed to be used.

**Automated coverage:** `Fhir_Observation_Search_500s_As_Soon_As_Any_Vitals_Record_Exists_Due_To_Missing_Uuid_Mapping_Backfill` (`ObservationApiTests.cs`) creates a real patient/encounter via the REST API, seeds one realistic `form_vitals`+`forms` pair via direct insert (the same fixture-seeding pattern already established for Condition/Immunization/Procedure/Prescription, since the real `POST .../vital` REST endpoint requires an `encounters`/`notes` ACL grant this project's default admin test token doesn't have — a separate, minor gap not otherwise pursued here), and asserts both a patient-scoped category search and a completely unfiltered search return `500`. The test cleans up its own `form_vitals`/`forms` rows in a `finally` block — required, not just tidy, since this defect means any leaked vitals fixture row would permanently break every other Observation test (including the existing bare-bundle happy-path test) for the life of the shared dev database; confirmed live that the happy-path test fails while the fixture row exists and passes again immediately once it's cleaned up.
